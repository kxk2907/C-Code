...
public class UDPClient extends Canvas{
	public static void main(String args[]) throws Exception
	{
		...
		UDPClient tc = new UDPClient("client");
		...
		byte[] receiveData = new byte[100];
		 while(!UDPClient.readyToSend) {
         	Thread.sleep(2500);
         }
		String sentence = new String(SEND_OUT);
		sendData = sentence.getBytes();
		DatagramPacket sendPacket =	new DatagramPacket(sendData, sendData.length,IPAddress, 4445);   //UDP socket for the client
		...
		String s2 = modifiedSentence.substring(0, 4);
		String s1[] = s2.split(",");
        if(Running == false)					
		{
			tc.clear();
	...
	public UDPClient(String WhoAmI) {
		this.WhoAmI = WhoAmI;
