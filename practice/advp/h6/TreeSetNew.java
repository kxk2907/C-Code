 ...
 * TreeSetNew class 
 */
public class TreeSetNew implements AnotherTreeSet,MyIterator  {
	TreeNode treenode;
	static int runner = 0;
	static IteratorList it1 = new IteratorList(null,null);
	//default constructor
	public TreeSetNew() {
	}
...
//to find the size of the tree
	...
	public static void main(String args[]) throws IOException {
		...
		while (true) {
			...
			System.out.println("7. Display tree in a sorted order");              /// Implemented using iterator function
			...
			if(choice.equals("1")) {
				it1 = null;
				it1 = new IteratorList(null,null);
				...
				System.out.println("Display in the tree !!");
				while(tsn.hasNext() && runner < tsn.size()) {
					System.out.print(tsn.next()+ " ");
				}
				runner = 0;
			}
	...
	public IteratorList getLeast(TreeNode parent,TreeNode traverse,IteratorList it1) {   // using the listiterator created a list class with linked list
		if(traverse != null) {
		}
		if (traverse.getLeft() != null) {
			parent = traverse;
			traverse = traverse.getLeft();
			it1 = getLeast(parent,traverse,it1);
			it1 = it1.putInto(traverse.getValue(), it1);
			traverse = parent;
		}
		if(traverse.getRight() != null) {
			it1 = it1.putInto(traverse.getValue(), it1);
	...
	public IteratorList iterator() {  // iterator implementation
		TreeNode traverse = this.getRoot();
		TreeNode parent = this.getRoot();
		if(parent.value == null && parent.left == null && parent.right == null)
			return null;
		return(getLeast(parent,traverse,it1));
	}
	
	public boolean hasNext() {	//hashNext
		IteratorList temp1 = it1;
		if(temp1.element == null)
			return false;
		else 
			return true;
	}
	public Object next() {  // get next 
		IteratorList temp1 = it1;
		for(int i = 0;i<runner;i++)
			temp1 = temp1.getNext();
		Object ele = temp1.element;
		runner ++;
		return (ele);
	}
	public void remove1(Object ele) { // remove from iterator list
		it1 = it1.remove(ele, it1);
		return;
	}
}
