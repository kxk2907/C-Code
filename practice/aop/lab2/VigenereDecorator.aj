...
public aspect VigenereDecorator {
	...
	static Integer marker;
	static Integer globalC2 = 0;
	/**
	 * 
	 * @param args
	...
	void around() throws IOException: executeReset() && target(Reader) {
		globalC2 = marker;
		proceed();
	}
	...
	public static int decoder(int c) {
		if(c >= 65 && c<= 90) {
			int c2  = keyword.toUpperCase().charAt(globalC2);
			c = (c - 65) - (c2 - 65);
			while( c < 0)
 				c = c + 26;
			c = c % 26;
			...
			int c2  = keyword.toLowerCase().charAt(globalC2);
			c = (c - 97) - (c2 - 97);
			while( c < 0)
				c = c + 26;
			c = c % 26;
		...
		while(globalC2 >= keyword.length())
			globalC2 = globalC2 % keyword.length();
